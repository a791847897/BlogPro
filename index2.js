...
casper.then(function () {
    ...
    var collect = casper.evaluate(function () {
        ...
        $('.sidebar').children('section').eq(1).children('ul').children('li').each(function (index, element) {
            var sidebar2 = {};
            sidebar2.title2 = $(element).children('a').text();
            sidebar2.title2url = $(element).children('a').attr('href');
            array2.push(sidebar2);
        });
        $('.sidebar').children('section').eq(1).children('ul').children('li').each(function (index, element) {
            var sidebar3 = {};
            sidebar3.title3 = $(element).children('a').text();
            sidebar3.title3url = $(element).children('a').attr('href');
            array2.push(sidebar3);
            console.log(sidebar3);
        });
        pager.sidebar = array2;
        return pager;
    });
    DataAll = collect;
});
